...
foreach($KB in Get-Content .\INPUT\GroupA.txt) {
    $update=$wsus.SearchUpdates($KB)
    if(!$update){
        Write-Host "!! $KB for GROUPA_Computers not found !!" -ForegroundColor Red
...
Write-Host "Searching and approving updates..." -ForegroundColor DarkCyan
foreach($KB in Get-Content .\INPUT\GroupB.txt) {
    $update=$wsus.SearchUpdates($KB)
    if(!$update){
        Write-Host "!! $KB GROUPB_Computers not found !!" -ForegroundColor Red
        Write-Host "!! Search on WSUS and Approve manually !!" -ForegroundColor Red
    }
    else {
